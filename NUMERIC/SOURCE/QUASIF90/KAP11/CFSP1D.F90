      SUBROUTINE CFSP1D (N, XN, FN, W, ALPHA, BETA, MREP, A, B, C, D, H,&
      H1, H2, DM, DU1, DU2, RS, IERR)                                   
!                                                                       
!*****************************************************************      
!                                                                *      
!  CFSP1D computes the coefficients A(I), B(I), C(I), D(I),      *      
!  I=0, 1. ..., N-1, of a cubic fitting spline with prescribed   *      
!  first end point derivative.                                   *      
...
!                                                                *      
!                                                                *      
!  REMARK:  CFSP1D should not be called directly, but rather via *      
...
!                                                                *      
!  ALPHA :  first end point derivative at XN(0)                  *      
!  BETA  :  first end point derivative at XN(N)                  *      
!                                                                *      
...
!                                                                       
      ...
      DM (1:N - 1), DU1 (1:N - 1), DU2 (1:N - 1), RS (1:N - 1), SF (10) 
      SAVE SF 
!                                                                       
...
!                                                                       
   ...
   50    END DO 
!                                                                       
!       Adjust the outside corner elements of the matrix using the      
!       auxiliary variables. Their values will be needed in the         
!       second pass for parametric splines, hence we will store them    
!       in auxilary varibles secured via SAVE commands.                 
!                                                                       
         SF (1) = H (0) - B (0) * C (0) - B (1) * H2 (0) * H1 (0) 
         SF (2) = B (1) * H1 (0) * H1 (1) 
         SF (3) = B (N - 1) * H1 (N - 2) * H1 (N - 1) 
         SF (4) = H (N - 1) - B (N - 1) * H2 (N - 2) * H1 (N - 1)       &
         - B (N) * C (N - 1)                                            
         SF (5) = H1 (0) * (B (1) + B (0) ) + 2.0D0 * H (0) * H (0) 
         SF (6) = H1 (N - 1) * (B (N) + B (N - 1) ) + 2.0D0 * H (N - 1) &
         * H (N - 1)                                                    
         SF (7) = B (1) * H2 (0) + B (0) * H1 (0) - H (0) * H (0) 
         SF (8) = B (N - 1) * H2 (N - 2) + B (N) * H1 (N - 1) - H (N -  &
         1) * H (N - 1)                                                 
         SF (9) = B (1) * H1 (1) 
         SF (10) = B (N - 1) * H1 (N - 2) 
!                                                                       
...
!                                                                       
      ...
      C (N) = (DUMMY4 + C (N - 1) * SF (8) - C (N - 2) * SF (10) )      &
      / SF (6)                                                          
!                                                                       
